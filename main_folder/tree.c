...
// Generate and return a new label number just for AST dumping.
static int gendumplabel(void){
  static int id = 1;
  return id++;
}

// Given an AST tree, print it out and follow the traversal of tree that genAST() follow.
// This function is written only for visualization process. Just to see how to right associativity works.
void dumpAST(struct ASTnode *n,int label,int level){
  ...
  switch(n->op){
     case A_IF:
	  ...
	  fprintf(stdout, "\n");
	  dumpAST(n->left,Lfalse,level+2);// Conditional part
	  dumpAST(n->mid,NOLABEL,level+2);// Conditional true.
	  if(n->right){
            // If there is a else part then 
