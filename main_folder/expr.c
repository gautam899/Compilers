...
// Parse the index into an array and return
...
static struct ASTnode *array_access(void){
  ...
  // Scale the index by the size of the element's type. Why scale when the left is just a node with the address to the base of the array and right node has the expression type??
  right = modify_type(right, left->type, A_ADD);

  // Return an AST tree where the array's base had an offset added to it, and dereferenced the element. Still an lvalue at this point.
...
// Parse a postfix expression and return an AST node reprsenting it. The identifier is already in Text.
...
struct ASTnode *prefix(void) {
  ...
  switch (Token.token) {
	...
	fatal("* operator must be followed by an identifier or *");
      ...
      break;
    case T_MINUS:
      // Scan the next token and parse it recurcively as a prefix expression.
      scan(&Token);
      tree = prefix();

      // Prepend a A_NEGATE operation to the tree and make then child an rvalue. Because the char's are unsigned, also widen this to int so that its signed.
      tree->rvalue = 1;
      tree = modify_type(tree, P_INT, 0);
      tree = mkastunary(A_NEGATE, tree->type, tree, 0);
      break;
    case T_INVERT:
      // Get the next token and parse it recurcively as a prefix operation
      scan(&Token);
      tree = prefix();
